	...
	public void inputToView(List<FeatureContainer> featureContainers) {
		...
		try {
			...
			try(BufferedReader stdInput = new BufferedReader(new InputStreamReader(p.getInputStream()))){
				String line = null;
				while((line = stdInput.readLine()) != null)
					diffInfo.add(new Triple<String, Project, List<String>>(line, ProjectStore.getActiveProject().clone(), new ArrayList<>()));
			}
			...
			for(Triple<String, Project, List<String>> entry : diffInfo) {		
				...
				for(String diff : diffs) {
					...
					IFile file = project.getIProject().getWorkspace().getRoot().getFile(new Path("/" + project.getID() + "/" + info[1]));
					
					System.out.println("Fileeee: " + file.getLocation().toString());
					
					if(info[0].equals(GIT_ADDED)) {
						// If the diff show that the file has been added then it needs to be removed from this project
						project.getFeatureContainers().stream()
													  .filter(featureContainer -> featureContainer.isMappedIn(file) || featureContainer.isAnnotatedIn(file))
													  .forEach(featureContainer -> featureContainer.removeFile(file));
						
					} else if(info[0].equals(GIT_DELETED) || info[0].equals(GIT_MODIFIED)) {
						// If the diff showd that the file was removed/modified then we need to reparse it.
						ParseJob job = new ParseJob("Parse history", project, file, Display.getDefault().getActiveShell());
						job.setUser(true);
						job.schedule();
					}
				}
				...
				System.out.println("Command " + commandToExecute.toString());
				p = Runtime.getRuntime().exec(commandToExecute.toString());
			}
		} catch(IOException e) {
			e.printStackTrace();
		...
		IntStream.range(0, diffInfo.size()).forEach(i -> {	
			FeatureContainer featureContainer = diffInfo.get(i).getCenter().getFeatureContainer(featureContainers.get(0).getFeature());
		...
		allTraces.entrySet().stream().forEach(entry -> {
			...
			// When all data has been collected set the data points with the correct data provider
